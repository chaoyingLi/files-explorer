...
- 树与预览区之间 **可拖拽调整宽度**
- 同文件不重复创建窗口，自动聚焦已有
- **独立工具栏** — 打开 / 打印 / 另存为 / 删除 / 复制路径 / 在访达中显示
- **自定义标题栏** — 与主窗口一致，支持拖拽和双击最大化
- **右键「新窗口预览」** — 主程序右键菜单入口

### 📂 「此电脑」页面
- **驱动器和文件夹卡片** — 磁盘空间进度条、系统特殊目录
- **驱动器自动检测** — 每 3 秒轮询，U 盘插入 / 弹出自动刷新
- **最近访问列表** — 名称 / 类型 / 路径 / 时间四列，双击导航
- 相对时间显示（刚刚 / 分钟前 / 小时前 / 天前）
- 最多 50 条，自动去重置顶，`localStorage` 持久化

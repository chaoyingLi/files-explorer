...
- **🧠 智能** — 50+ 语言代码高亮、Office/PDF/视频/归档预览、通配符搜索、系统托盘常驻
...
| **独立预览窗口** | 65%×75% 自适应尺寸，文件树导航 + 工具栏（打开/重命名/另存为/删除/复制路径/定位）|
...
| **动态切换** | 窗口可见时显示「隐藏主窗口」，隐藏时显示「显示主窗口」|
| **快速访问** | 下载/文档一键直达 |
| **托盘行为** | 关闭窗口 → 隐藏到托盘（可设退出）|
| **单击/双击** | 单击 toggle 显隐 / 双击强制显示+聚焦 |
| **清理缓存** | 托盘菜单 + 设置关于页双入口，自定义确认对话框 |
| **Dock/任务栏** | 点击重新显示主窗口（macOS Dock / Windows 任务栏）|
...
| **关于** | 版本号 / 轻量·极速·现代·智能 pill 标签 / 清理缓存 |
...
- 💾 **会话持久化** — `tauri-plugin-window-state` 窗口几何自动保存恢复 + 布局/Tab/历史/面板全部恢复，三条退出路径全覆盖
- 🧹 **缓存清理** — 一键重置所有缓存数据，仅保留设置 |
- ⌨️ **30+ 快捷键** — Enter/Ctrl+CXV/F2/F5/Ctrl+NZ/Ctrl+W...
- 🌐 **国际化** — 中文/English，200+ 翻译 Key
- 🔄 **自动更新** — `tauri-plugin-updater` 就绪（暂缓启用）
...
- 🪟 窗口状态：`tauri-plugin-window-state` 自动保存恢复窗口几何
- 💾 会话持久化：三条退出路径全覆盖（beforeunload / tray-hide / tray-quit）
- 🔍 搜索历史：最近 15 条持久化，↑↓ 选择，去重
- 📋 右键菜单：复制路径功能，逻辑分组重构
- 🖥️ 托盘菜单：纯文字无图标，显示/隐藏动态切换
- 🔧 自启动优化：移除 store init 冗余写入
- 🖼️ 视频编码检测：不支持编码自动提示外部播放器
- 📐 预览窗口：65%×75% 自适应尺寸
- 🧹 清理缓存：托盘+设置双入口，自定义确认对话框，清除 session/搜索/视频位置/窗口状态/收藏夹
- 🖱️ 托盘双击：双击强制显示主窗口，单击保持 toggle
- 🍎 Dock 恢复：macOS Dock 点击 / Windows 任务栏点击恢复显示
- ⌨️ 键盘导航：↑↓←→Home/End/PgUp/PgDn 文件列表导航，Shift 范围多选
- 🌳 树形键盘导航：独立预览窗口左侧树支持全键盘操作，与右侧预览联动
- 📂 收藏夹优化：右键标签动态切换「添加到/从收藏夹移除」，确认对话框改用 Tauri ask
- 🗂️ 路径归一化：全量修复跨平台路径分隔符问题（存储/比较/显示/Rust 系统调用）
- 🧭 面包屑：路径统一使用 / 跨平台格式

// platform/linux/path_impl.rs
// Linux path provider implementation.
...
impl PlatformPath for PathImpl {
    fn app_data_dir(&self) -> PathBuf {
        home().join(".local").join("share").join("files-explorer")
    }

    fn app_cache_dir(&self) -> PathBuf {
        home().join(".cache").join("files-explorer")
    }

    fn app_config_dir(&self) -> PathBuf {
        home().join(".config").join("files-explorer")
    }
